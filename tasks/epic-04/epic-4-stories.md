...
| --- | --- | --- | --- |
| 4.1 | Cursor list. | listUrls(limit, cursor) used. Page shows items. Button Mais visible iff hasMore. Next call sends nextCursor. Query keys include cursor or use an infinite query. | LinkListResponse |
| 4.2 | Empty and pending. | Pending: not a flash of empty. Empty: "Nenhum link ainda" (or equivalent) + link to Home. | LinksPage |
