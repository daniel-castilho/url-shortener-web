...
| 3.3 | 429 shows wait time when the backend sends it. | ApiError can carry retryAfterSec parsed from Retry-After. Copy: "Too many requests. Try in Ns." when N is a number; else mapApiError(429). Other statuses unchanged. | Audit 429 gap |
| 3.4 | Success is usable. | After 201, shortUrl is on screen. Button Copy writes it to clipboard. Failure to copy shows a line of text, not an alert storm. | Product |
