...
| 2.3 | ErrorBoundary on the shell. | Class or react-error-boundary around AppRoutes (or equivalent). Fallback is English, has retry that resets. Does not wrap the whole AuthProvider if that hides nav on purpose — pick one; document it. grep confirms a boundary exists. | Audit red flag 2 |
