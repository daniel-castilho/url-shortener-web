...
- `/` → static SPA (`try_files $uri /index.html` only under app paths)
- `/api/` → Java upstream
- `/actuator` → Java upstream (if exposed)
- `/{id}` → Java upstream (redirect). Do not fall back to `index.html` here.
